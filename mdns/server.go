...
func NewServer(config *Config) (*Server, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Create new IPv4 get error %s", err)
	}
	fmt.Println("Create new IPv4 pass")
	ipv6List, err := net.ListenMulticastUDP("udp6", config.Iface, ipv6Addr)
	if err != nil {
		fmt.Println("Create new IPv6 get error", err)
	}
	fmt.Println("Create new IPv6 pass")
